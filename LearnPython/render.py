...
def main(arg):
    ...
    num_of_images = 0
    for root, dirs, files in os.walk(arg.output_folder):
        for file in files:
            if file.endswith(".png"):
                num_of_images += 1
        break
    if num_of_views == num_of_images:
        print('[INFO] skip rendering.')
        return
    ...
    for i, view in enumerate(views):
        ...
        all_images_exist = os.path.exists(bpy.context.scene.render.filepath)
        
        for name, output in outputs.items():
            output.file_slots[0].path = os.path.join(arg.output_folder, f'{i:03d}_{name}')
            all_images_exist = all_images_exist and os.path.exists(output.file_slots[0].path)
    ...
    if arg.is_debug:
        debugBlenderPath = os.path.join(arg.output_folder, 'debug.blend')
...
class RendererInputConfig:
    def __init__(self, jsonData):
        self.views = jsonData["views"]
        self.object = jsonData["object"] if "object" in jsonData else ''
        self.output_folder = jsonData["output_folder"] if "output_folder" in jsonData else '/tmp'
        self.resolution = jsonData["resolution"] if "resolution" in jsonData else 512
        self.engine = jsonData["engine"] if "engine" in jsonData else "CYCLES"
        
        self.geo_mode = jsonData["geo_mode"] if "geo_mode" in jsonData else False
        self.save_depth = jsonData["save_depth"] if "save_depth" in jsonData else False
        self.save_normal = jsonData["save_normal"] if "save_normal" in jsonData else False
        self.save_albedo = jsonData["save_albedo"] if "save_albedo" in jsonData else False
        self.save_mist = jsonData["save_mist"] if "save_mist" in jsonData else False
        self.split_normal = jsonData["split_normal"] if "split_normal" in jsonData else False
        self.save_mesh = jsonData["save_mesh"] if "save_mesh" in jsonData else False
        
        self.save_debug_file = jsonData["save_debug_file"] if "save_debug_file" in jsonData else False
        
    def __str__(self):
        return f"views:{self.views}\nobject:{self.object}\noutput_folder:{self.output_folder}\nresolution:{self.resolution}\
        \nengine:{self.engine}\ngeo_mode:{self.geo_mode}\nsave_depth:{self.save_depth}\nsave_normal:{self.save_normal}\
        \nsave_albedo:{self.save_albedo}\nsave_mist:{self.save_mist}\nsplit_normal:{self.split_normal}\nsave_mesh:{self.save_mesh}"
...
if __name__ == '__main__':
    ...
    configIndex = sys.argv.index("render.py") + 1
    configFilePath = sys.argv[configIndex]
    configFile = open(configFilePath, "r", encoding='utf-8')
    configStr = configFile.read()
    configFile.close()
    
    jsonData = json.loads(configStr)
    configData = RendererInputConfig(jsonData)
    
    inputFilePath = sys.argv[configIndex+1]
    configData.object = inputFilePath
    
    if len(sys.argv) > configIndex+2:
        inputFileRoot = sys.argv[configIndex+2]
        if not inputFileRoot.endswith("\\"):
            inputFileRoot += "\\"
        
        subOutputPath = configData.object.replace(inputFileRoot, "")
        subOutputPath = subOutputPath.replace("\Scan\Scan.obj", "")
        configData.output_folder = os.path.join(configData.output_folder, subOutputPath)
